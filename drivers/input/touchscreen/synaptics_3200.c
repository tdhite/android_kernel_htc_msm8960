...
/* 1 -> pocket wake protection on;
 * 2 -> pocket wake protection with only near check, no dark check;
 * 0 - off.
 */
int pwp_switch = 1;

/* 0 -> logo used as power off on long tap,
 *   and short tap syncs input on/off at same time;
 * 1 -> logo used as full menu button,
 *   sync on/off events separately.
 */
int l2m_2_phase = 0;
...
/* 0 -> No wake;
 * 1 -> Home2Wake;
 * 2 -> Home2Wake/Logo2Sleep if l2m_switch = 0;
 * 3 -> Logo2wake/Logo2sleep if l2m_switch = 0
 */
int h2w_switch = 0;
int h2w_temp = 0;
...
/* if 1 -> Logo2Sleep/Logo2Wake will wait for long tap;
 * if 0 -> it won't wait for no jiffies.
 */
int logo_delay_switch = 1;

/* length of vibration in msec/5 - set 0 to deactivate it;
 *   1 -> 5, 2 -> 10, 6 -> 30, 9 -> 45 (max)
 */
int sleep_wake_vibration_time = 6;
...
static void sweep2wake_presspwr(struct work_struct * sweep2wake_presspwr_work) {
	/* don't wake if in pocket */
	if ( scr_suspended == true && pwp_switch >= 1 && power_key_check_in_pocket((pwp_switch == 1)?1:0) ) return;
...
/* If finger is released, set this to 1, which means longtap count can begin.
 * If user keeps finger on screen, dont allow it after one count went down...
 */
...
static void sweep2wake_longtap_count(struct work_struct * sweep2wake_longtap_count_work) {
	...
	if (!break_longtap_count)
	{
		...
		else if ( scr_suspended == true || ( is_wake_option_set() && ( l2m_switch == 0 || (l2m_switch == 1 && l2m_2_phase == 0) ) ) )
		{
			/* screen is off, or wake option is set with logo2menu is not used, or logo2menu is set but not 2_phase */
...
{
 	/* s2w and doubletap2wake should not wake on logo */
    if ((s2w_switch > 0 || dt2w_switch > 0) && scr_suspended == true) return 0;

 	/* logo2sleep and logo2menu is both off, so don't report logo area! */
    if (h2w_switch < 2 && l2m_switch == 0) return 0;
...
static void synaptics_ts_finger_func(struct synaptics_ts_data *ts)
{
	...
	/* this line is to avoid setting -10/-10 for coordinates,
	 * when wake option is not set, so we shouldn't modify coordinates
	 */
	if (!is_wake_option_set()) ts_is_on = 1;
		...
		if (finger_pressed == 0) {
			...
			if ((((ts->finger_count > 0)?1:0) == 0)) { 
				...
				printk(KERN_INFO "[home2wake/S2W]: Finger released, reseting vars. Last post %d %d\n",last_touch_position_x, last_touch_position_y);
				{
				    if (s2w_switch > 0 && scr_suspended == true)
				    {
				    ...
				    } else if (!between_screen_off_from_longtap_and_touch_release && dt2w_switch > 0 && scr_suspended == true) {
						...
						/* touch released after screen off from long tap, set variable to 0 (false) */
				    	between_screen_off_from_longtap_and_touch_release = 0;
							...
							if (scr_suspended == false)
							{
								...
								if (l2m_switch > 0)
								{
									if (report_ret == 3)
									{
										...
										/* commented - long tap time count worker used instead */
										/* sweep2wake_pwrtrigger(); */
									} else {
										/* two phase menu input sync (or no longtap menu sleep == no wake set),
										 * and was pressed, trigger menu 0
										 */
										if ((l2m_2_phase == 1 || !is_wake_option_set()) && menu_pressed == 1)
										{
											/* MENU */
											sweep2wake_menutrigger();
										} else if (l2m_2_phase == 0) {
 											/* no two phase menu input sync, just trigger menu */
											/* MENU */
											sweep2wake_menutrigger();
										}
									}
									...
									if (h2w_switch >= 2)
									{
										/* logo to wake (h2w_switch 3) or at least logo to sleep (h2w_switch 2) enabled.
										 * Screen is on, switch it off
                                         */
										if (report_ret >= 2 && logo_delay_switch == 0)
										{
											/* 2 or 3 = enough time passed for non-accidental tap and logo delay is off */
											/* OFF */
											sweep2wake_pwrtrigger();
										}
									}
								...
								{ /* logo to wake enabled. screen is off, switch it on */
									if (report_ret >= 2 && logo_delay_switch == 0)
									{
 										/* 2 or 3 = enough time passed for non-accidental tap, power on */
										/* ON */
										sweep2wake_pwrtrigger();
									}
								}
							...
							input_mt_sync(ts->input_dev);
								...
								if (scr_suspended == false)
								{
									if (h2w_switch >= 2)
									{
										...
										if (l2m_switch == 0 && logo_delay_switch == 0 && report_ret == 3)
										{
 											/* 3 = enough time passed for long tap, or logo delay is off */
											/* OFF  */

 											/* - don't sleep here, on short tap, only sleep when user finger left touchscreen */ 
											/* sweep2wake_pwrtrigger(); */
										} else if (l2m_2_phase == 1 || logo_delay_switch == 1) {
											if (l2m_2_phase == 1)
											{
												menu_pressed = 0;
												/* MENU event -> 1 */
									...
									} else if (l2m_switch > 0 && is_wake_option_set() && logo_delay_switch == 1) {
...
static void synaptics_ts_button_func(struct synaptics_ts_data *ts)
{
	...
	if (data) {
		...
		if (dt2w_switch == 0 && home_key_pressed && h2w_switch > 0 && h2w_switch != 3)
		{
			/* if H2w not off (0) , or not Logo2Wake (3), Home should trigger screen on */
			if (scr_suspended == true)
			{
